...
typedef struct USB_OTG_ep
{
  uint8_t        num;								//端点num
  uint8_t        is_in;							//是否为输入
  uint8_t        is_stall;  				//是否挂起
  uint8_t        type;							//端点类型
  uint8_t        data_pid_start;		//起始包PID
  uint8_t        even_odd_frame;		//奇偶帧
  uint16_t       tx_fifo_num;				//发送fifo编号
  uint32_t       maxpacket;					//最大包
  /* transaction level variables*/
  uint8_t        *xfer_buff;				//传输buff
  uint32_t       dma_addr;  
  uint32_t       xfer_len;					//传输数据长度
  uint32_t       xfer_count;				//传输数据计数
...
{
  uint8_t       host_channels;		//主机通道
  uint8_t       dev_endpoints;		//设备端点
  uint8_t       speed;						//速度
  uint8_t       dma_enable;				//使能dma
  uint16_t      mps;							//最大包
  uint16_t      TotalFifoSize;		//总FIFO大小
  uint8_t       phy_itface;				//物理接口
  uint8_t       Sof_output;				//SOF脉冲输出
  uint8_t       low_power;				//掉电
  uint8_t       coreID;						//核心ID--FS/HS
...
typedef struct _DCD
{
  ...
  USB_OTG_EP     in_ep   [USB_OTG_MAX_TX_FIFOS];	//输入端点数组
  USB_OTG_EP     out_ep  [USB_OTG_MAX_TX_FIFOS];	//输出端点数组
